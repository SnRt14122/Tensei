...
import { BookOpen, Brain, CheckCircle2, ChevronDown, ChevronLeft, ChevronRight, Download, Languages, Search, Target, X } from "lucide-react";
...
import { buildRecallDeck, type RecallItem } from "@/lib/activityPractice";
import { ActivityWeek } from "./ActivityWeek";
import "./ActivityPractice.css";

const PAGE_SIZE = 10;
const RecallSession = dynamic(() => import("./RecallSession"), { ssr: false });

function DaySection({ day, mode, defaultOpen, onRecall }: { day: ActivityDay; mode: ActivityMode; defaultOpen: boolean; onRecall: (items: RecallItem[]) => void }) {
  const [open, setOpen] = useState(defaultOpen);
  const recallItems = useMemo(() => buildRecallDeck([day]), [day]);
  ...
  return <article className="activity-day">
    ...
    {open && <div className="activity-grid" id={`activity-${day.key}`}>
      {recallItems.length > 0 && <div className="activity-day-actions"><button className="activity-export" onClick={() => onRecall(recallItems)}><Brain size={15} />自测当天内容</button></div>}
...
export function ActivityDashboard({ days, reviewQueue, todayKey }: { days: ActivityDay[]; reviewQueue: ReviewItem[]; todayKey: string }) {
  ...
  const [session, setSession] = useState<RecallItem[] | null>(null);
  const visibleDays = useMemo(() => {
    const today = new Date(`${todayKey}T00:00:00Z`);
    const cutoff = range === "all" ? "0000-00-00" : new Date(today.getTime() - (range - 1) * 86400000).toISOString().slice(0, 10);
    return filterActivityDays(selectedDate ? days.filter(day => day.key === selectedDate) : days, cutoff, mode, deferredSearch);
  }, [days, range, todayKey, mode, deferredSearch, selectedDate]);
  const recallDeck = useMemo(() => buildRecallDeck(visibleDays), [visibleDays]);
  ...
  function changeRange(value: 7 | 30 | "all") { setRange(value); setPage(0); setSelectedDate(null); }
  function changeMode(value: ActivityMode) { setMode(value); setPage(0); }
  function selectDate(key: string) {
    setSelectedDate(key); setRange("all"); setMode("all"); setSearch(""); setPage(0);
  }
  function openRecall(items: RecallItem[]) { if (items.length) setSession(items); }
  ...
  return <>
    <ActivityWeek days={days} todayKey={todayKey} selected={selectedDate} onSelect={selectDate} />
    {selectedDate && <div className="activity-date-filter"><span role="status">{selectedDate} 的学习记录</span><button aria-label="清除日期筛选" title="清除日期筛选" onClick={() => setSelectedDate(null)}><X size={16} /></button></div>}
    <div className="activity-practice-launch"><span>当前筛选：{recallDeck.length} 项可自测内容</span><button className="activity-export" disabled={!recallDeck.length} onClick={() => openRecall(recallDeck)}><Brain size={16} />开始回想自测</button></div>
    ...
    {visibleDays.length ? <div className="activity-list">{pageDays.map((day, index) => <DaySection key={`${selectedDate ?? "range"}:${day.key}`} day={day} mode={mode} defaultOpen={index === 0} onRecall={openRecall} />)}</div> : <section className="activity-empty-state"><Search size={28} /><h2>{selectedDate && !days.some(day => day.key === selectedDate) ? "当天暂无已保存记录" : "没有匹配的记录"}</h2></section>}
    ...
    {session && <RecallSession items={session} onClose={() => setSession(null)} />}
  </>;
}
